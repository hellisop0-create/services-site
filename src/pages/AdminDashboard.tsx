...
import { db, auth } from '../firebase/config'; // Ensure auth is exported from config
import { UserProfile, WorkerProfile } from '../types';
import { Users, Hammer, Shield, Trash2, CheckCircle, XCircle, Search, LayoutDashboard, MessageSquare } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import ChatBox from './ChatBox'; // Make sure this file exists in the same folder

const AdminDashboard: React.FC = () => {
  const [users, setUsers] = useState<UserProfile[]>([]);
  const [workers, setWorkers] = useState<WorkerProfile[]>([]);
  const [loading, setLoading] = useState(true);
  // ADDED 'chats' to the type below
  const [activeTab, setActiveTab] = useState<'stats' | 'users' | 'workers' | 'settings' | 'chats'>('stats');
  ...
  useEffect(() => {
    ...
    // 4. NEW: Listen to all platform chats
  ...
  if (loading) return <div className="p-10 text-center font-black uppercase tracking-widest text-primary">Loading admin data...</div>;
    ...
    <div className="min-h-screen">
      <div className="bg-primary text-white py-4 border-b border-white/10 hidden md:block">
        <div className="max-w-7xl mx-auto px-6 flex gap-12 font-bold text-xs uppercase tracking-widest">
           ...
           <div className="flex items-center gap-2">
             <span className="opacity-50">Active Chats:</span>
             <span className="text-accent">{stats.activeChats}</span>
           </div>
           <div className="flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 pb-6 border-b border-high-border">
          ...
          <div className="flex bg-white border border-high-border p-1 rounded-xl shadow-sm">
            {(['stats', 'users', 'workers', 'chats', 'settings'] as const).map((tab) => (
              <button
                key={tab}
        ...
        <AnimatePresence mode="wait">
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm">
                    {filteredUsers.map(u => (
                      <tr key={u.uid} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <select 
                            ...
                            className={`px-2 py-1 rounded-lg text-xs font-bold uppercase outline-none cursor-pointer border border-transparent focus:border-secondary transition-all ${
                              ...
                              u.role === 'worker' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-100'
                            }`}
                          >
                        ...
                        <td className="px-6 py-4 text-slate-500">{u.createdAt?.toDate().toLocaleDateString() || 'N/A'}</td>
                        <td className="px-6 py-4">
          ...
          {activeTab === 'workers' && (
            <motion.div key="workers" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="space-y-6">
                ...
                {filteredWorkers.map(w => (
                  <div key={w.uid} className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col md:flex-row gap-6 shadow-sm overflow-hidden relative">
                    ...
                    <div className="flex-grow space-y-2">
                      ...
                      <button onClick={() => setEditingWorker(w)} className="text-[10px] font-black text-secondary uppercase tracking-widest hover:underline">Quick Edit Details</button>
                    </div>
                    <div className="flex md:flex-col gap-2 items-center justify-center border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                      <button onClick={() => handleApproveWorker(w.uid)} disabled={w.status === 'approved'} className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-600 rounded-xl font-bold text-sm hover:bg-green-100 disabled:opacity-50 transition-colors w-full"><CheckCircle className="h-4 w-4" /> Approve</button>
                      <button onClick={() => handleRejectWorker(w.uid)} disabled={w.status === 'rejected'} className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-xl font-bold text-sm hover:bg-red-100 disabled:opacity-50 transition-colors w-full"><XCircle className="h-4 w-4" /> Reject</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* CHATS TAB INTEGRATION */}
          {activeTab === 'chats' && (
            <motion.div key="chats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1 space-y-4">
                <h3 className="text-xs font-black text-primary uppercase tracking-widest">Live Platform Conversations</h3>
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2">
                  {allChats.map(chat => (
                    <button key={chat.id} onClick={() => setActiveChatId(chat.id)} className={`w-full p-4 text-left rounded-2xl border transition-all ${activeChatId === chat.id ? 'border-secondary bg-secondary/5 ring-1 ring-secondary' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-[10px] font-black text-secondary uppercase tracking-tighter">ID: {chat.id.slice(0,8)}</span>
                        <MessageSquare size={14} className={activeChatId === chat.id ? 'text-secondary' : 'text-slate-300'} />
                      </div>
                      <p className="text-xs font-bold text-primary truncate">{chat.workerName || 'Worker'} & {chat.clientEmail?.split('@')[0] || 'Client'}</p>
                    </button>
                  ))}
                  {allChats.length === 0 && <p className="text-xs text-high-muted italic">No chats currently active.</p>}
                </div>
              </div>
              <div className="lg:col-span-2">
                {activeChatId ? (
                  <ChatBox chatId={activeChatId} currentUser={users.find(u => u.uid === auth.currentUser?.uid) || { uid: auth.currentUser?.uid, role: 'admin', email: auth.currentUser?.email }} />
                ) : (
                  <div className="h-[500px] border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-slate-400 bg-white/50">
                    <MessageSquare size={48} className="mb-4 opacity-10" />
                    <p className="text-xs font-black uppercase tracking-widest opacity-40">Select a thread to begin supervision</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeTab === 'settings' && (
            <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl bg-white rounded-2xl border border-high-border p-8 shadow-sm space-y-6">
              <div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <textarea className="w-full px-4 py-3 bg-high-bg border border-high-border rounded-xl text-sm outline-none focus:ring-2 focus:ring-secondary min-h-[100px]" placeholder="Enter message..." value={globalBanner} onChange={(e) => setGlobalBanner(e.target.value)} />
                  </div>
                  <button onClick={handleUpdateSettings} className="bg-primary text-white px-8 py-3 rounded-xl font-bold text-sm hover:translate-y-[-1px] transition-all shadow-md">Save Platform Settings</button>
                </div>
              </div>
              <div className="pt-6 border-t border-high-border">
                ...
                <button onClick={promoteToAdmin} className="border border-red-200 text-red-600 px-6 py-2 rounded-lg text-xs font-bold hover:bg-red-50 transition-colors">Promote User to Super Admin</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>

        {/* Edit Worker Modal (Existing logic) */}
        <AnimatePresence>
          {editingWorker && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl border border-high-border shadow-2xl w-full max-w-lg p-8">
                ...
                <form onSubmit={handleSaveWorker} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input className="w-full px-3 py-2 bg-high-bg border border-high-border rounded-lg text-sm outline-none" value={editingWorker.name} onChange={(e) => setEditingWorker({...editingWorker, name: e.target.value})} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-high-muted uppercase">Category</label>
                      <input className="w-full px-3 py-2 bg-high-bg border border-high-border rounded-lg text-sm outline-none" value={editingWorker.category} onChange={(e) => setEditingWorker({...editingWorker, category: e.target.value})} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-high-muted uppercase">Experience (Years)</label>
                    <input type="number" className="w-full px-3 py-2 bg-high-bg border border-high-border rounded-lg text-sm outline-none" value={editingWorker.experience} onChange={(e) => setEditingWorker({...editingWorker, experience: parseInt(e.target.value)})} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-high-muted uppercase">Public Bio</label>
                    <textarea className="w-full px-3 py-2 bg-high-bg border border-high-border rounded-lg text-sm outline-none min-h-[80px]" value={editingWorker.bio} onChange={(e) => setEditingWorker({...editingWorker, bio: e.target.value})} />
                  </div>
